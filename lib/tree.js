  ...
  partial_matching_child_node(parent_node, child_token_string, nodeindex, tripleindex, triple) {
    ...
    if (tripleindex !== triple.get_representation().length) {
      ...
      if (current_fragment.contents.length > this.max_fragment_size) {
        if (root_node.id == this.root_node_id) {
    ...
    if ( (! node.has_parent_node()) || node.get_parent_node().get_fragment_id() != fragment_id){
        ...
        for (var i = 0; i < childnodes.length; i++){
            if (childnodes[i].get_fragment_id() == fragment_id) {
                ...
                if (new_fragment.get_contents_size() > this.max_fragment_size){
        ...
        if (fragment.get_contents_size() > this.max_fragment_size){
...
//     childnode.change_fragment_node_and_children(current_fragment.id, new_fragment);
