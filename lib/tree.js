...
module.exports = class {
  ...
  balance(node) {
    ...
    if (node.get_parent_node().get_fragment_id == node.get_fragment_id){
      parent_node = node.get_parent_node()
    }
    let before_parent_node = null;
    while (parent_node !== this.get_root_node() && parent_node.get_child_count() < 2 && parent_node.get_parent_node().get_fragment_id() === fragment_id && parent_node.get_parent_node() !== this.get_root_node()) {
      before_parent_node = parent_node;
      parent_node = parent_node.get_parent_node();
    }
    console.log("BEFORE", parent_node.id, parent_node.get_fragment().id)
    console.log("BEFORE", node.id, node.get_fragment().id)
    ...
    this.fc.add_fragment(new_fragment)
    let childnode = node
