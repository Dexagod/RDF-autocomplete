...
module.exports = class {
  ...
  constructor(max_fragment_size, fragment_cache) {
    ...
    this.fc.add_fragment(root_fragment);
  ...
  async get_root_node(){
    let frag = await this.fc.get_fragment_by_id(this.root_fragment_id)
    console.log("ROOTNODE", frag.get_node_by_id(this.root_node_id))
    return frag.get_node_by_id(this.root_node_id);
  }


  async add_triple(triple) {
    let node = await this.get_root_node();
      ...
      if (children_token_strings.length !== 0) {
        ...
        for (var i = 0; i < children_token_strings.length; i++) {
          ...
          if (node_match_index === child_tokens_length) {
            ...
            found_child = true;
            node = await this.total_matching_child_node(node, children_token_strings[i]);
            break;
          } else if (node_match_index > 0) {
            // A partial match was found
            found_child = true;
            return await this.partial_matching_child_node(node, children_token_strings[i], node_match_index, index, triple);
          }
        }
        if (found_child !== true) {
          return await this.no_matching_child_node(node, index, triple);
        }
  ...
  async no_matching_child_node(node, index, triple) {
    // There are no children in this node
    let repr = triple.get_representation();
    let current_fragment = await node.get_fragment();
    ...
    if (current_fragment.get_contents_size() > this.max_fragment_size) {
      if (node.has_parent_node()){
        await this.balance(node)
      } else {
        await this.balance(child)
      }
      //IDEA CHILD WORKS
    }
    node = child;
  ...
  async partial_matching_child_node(parent_node, child_token_string, nodeindex, tripleindex, triple) {
    //IDEA Promise.all
    let node = await parent_node.get_child_by_token_string(child_token_string);
    let root_parent = node.get_parent_node();
    let children = await node.get_children_objects();
    let current_fragment = await node.get_fragment();
    ...
    if (tripleindex !== triple.get_representation().length) {
      ...
      await old_match_node.set_children(node.get_children());
  ...
  async total_matching_child_node(node, child_token_string) {
    return await node.get_child_by_token_string(child_token_string);
  }

  async search_triple(searched_triple) {
    ...
    while (current_index < searched_triple.get_representation().length) {
      ...
      //IDEA dont use children objects, only ask correct child
      let node_children = await node.get_children_objects();
  ...
  async balance(node) {
    let fragment = await node.get_fragment().catch(error => console.log(error));;


    let before_size = fragment.get_contents_size()
    console.log("BEFORE BALANCE SIZE:  id: ", fragment.id, "|",  fragment.get_contents_size())


    let fragment_id = node.get_fragment_id();
    let parent_node = node//.get_parent_node();
    if ( (await node.get_parent_node().catch(error => console.log(error))).get_fragment_id == node.get_fragment_id){
      parent_node = await node.get_parent_node().catch(error => console.log(error));
    }
    let before_parent_node = null;
    while (parent_node !== await this.get_root_node() && parent_node.get_child_count() < 2 && await(parent_node.get_parent_node()).get_fragment_id() === fragment_id && await parent_node.get_parent_node() !== await this.get_root_node()) {
      before_parent_node = parent_node;
      parent_node = await parent_node.get_parent_node().catch(error => console.log(error));;
    }
    console.log("BEFORE", parent_node.id, parent_node.get_fragment_id)
    console.log("BEFORE", node.id, node.get_fragment_id)
    let current_fragment = await parent_node.get_fragment().catch(error => console.log(error));
    // for (childindex in parent_node.get_children()){
    let new_fragment = new Fragment(this);
    await this.fc.add_fragment(new_fragment).catch(error => console.log(error));
    let childnode = node
    if (before_parent_node !== null) {
      childnode = before_parent_node
    }
    // Also updates fragment with nodes
    await childnode.change_fragment_node_and_children(current_fragment, new_fragment).catch(error => console.log(error));

    console.log("AFTER BALANCE SIZE :  id: ", fragment.id , " | ", fragment.get_contents_size())
    let after_size = fragment.get_contents_size();
    console.log("NEW FRAGMENT SIZE  :  id: ", new_fragment.id, " | ", new_fragment.get_contents_size())

    if (before_size === after_size){
      try {
        console.log("bpn", before_parent_node.id)
      } catch (e) {}
      console.log("pn", parent_node.id)
      console.log("cn", childnode.id)
      console.log("n", node.id)
      console.log("n", parent_node.get_fragment().id)
      console.log("n", node.get_fragment().id)
    }
