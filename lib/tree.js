...
module.exports = class {
  ...
  balance(node) {
    ...
    while ( node.has_parent_node() && node.get_parent_node().get_in_fragment_children_count() + node.get_in_fragment_children_count){
      return
    }
    let new_fragment = new Fragment(this);
    this.add_fragment(new_fragment);
    new_fragment.set_root_node(node)
    let total_changed_nodes = node.change_fragment_node_and_children(fragment_id, new_fragment);
    if (fragment.get_contents_size() > this.max_fragment_size){
        this.balance(node.get_parent_node())
    }
    let parent = node.get_parent_node()
    parent.propagate_in_fragment_children_count(-total_changed_nodes, parent.get_fragment_id())
