...
module.exports = class {
  ...
  // BALANCE THE ADDED NODE
  balance(node) {
    let current_frag_id = node.get_fragment_id()
    let new_fragment = new Fragment(this);
    this.add_fragment(new_fragment)
    node.change_fragment_node_and_children(current_frag_id, new_fragment)
...
//   while (parent_node.id !== this.root_node_id &&
//            parent_node.get_child_count() < 2 &&
//             parent_node.get_parent_node().get_fragment_id() === fragment_id && 
//              parent_node.get_parent_node().id !== this.root_node_id) {
