...
module.exports = class {
  ...
  constructor(max_fragment_size) {
    this.max_fragment_size = max_fragment_size;
    this.fragment_count = 0;
    this.node_count = 0;
    this.root_fragment = new Fragment(this);
    this.root_node = new Node("ROOT", this.root_fragment, null, this)
      ...
      if (children_tokenstring_array.length !== 0) {
        ...
        for (var i = 0; i < children_tokenstring_array.length; i++) {
          ...
          for (node_match_index;
            (node_match_index < child_tokens_length && current_child_token_array[node_match_index] === letter); node_match_index++) {//
              // console.log( (current_child_token_array[node_match_index] === letter)   ===   (current_child_token_array[node_match_index].localeCompare(letter, this.options.locale) === 0) )
            index += 1
            letter = repr[index];
          }
          ...
          if (node_match_index === child_tokens_length) {
            ...
            found_child = true;
            node = this.total_matching_child_node(node.get_children()[i]);
            break;
          } else if (node_match_index > 0) {
            // A partial match was found
            found_child = true;
            return this.partial_matching_child_node(node.get_children()[i], node_match_index, index, triple);
          }
        }
        if (found_child !== true) {
  ...
  partial_matching_child_node(node, nodeindex, tripleindex, triple) {
  ...
  total_matching_child_node(node) {
    return node;
  }

  search_triple(searched_triple) {
    let representation = searched_triple.representation;
    let node = this.root_node;
    let current_index = 0;
    while (current_index < searched_triple.representation.length) {
      ...
      for (var i = 0; i < node.get_children().length; i++) {
        let child = node.get_children()[i]
        let child_token_len = child.token_string.length;
        let representation_substring = searched_triple.representation.substring(current_index, current_index+child_token_len)
        // console.log((child.token_string.localeCompare(representation_substring)) === (child.token_string === representation_substring))
        if (child.token_string === representation_substring) {
          found = true;
          node = child;
  ...
  balance(node) {
    ...
    if (parent_node === this.root_node || parent_node.parent_node === this.root_node || parent_node.parent_node.fragment !== fragment) {
