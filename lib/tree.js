...
module.exports = class {
  ...
  adjust_root_node(new_fragment){
    return this.root_fragment_id = new_fragment.fragment_id;
  }
  ...
  add_triple(triple) {
    ...
    while (index < repr.length) {
      if (node.node_id == 0){
        let sugs = node.suggestions;
        node.fc = null
        node.suggestions = null;
        node.fc = this.fragment_cache
        node.suggestions = sugs;
      }
      ...
      if (children_token_strings.length !== 0) {
        ...
        for (var i = 0; i < children_token_strings.length; i++) {
          ...
          if (node_match_index === child_tokens_length) {
            ...
            found_child = true;
            node = this.partial_matching_child_node(node, children_token_strings[i], node_match_index, index, triple);
            node.add_triple(triple);
            return node
          }
        }
        if (found_child !== true) {
          node = this.no_matching_child_node(node, index, triple);
          node.add_triple(triple);
          return node
        }
  ...
  partial_matching_child_node(parent_node, child_token_string, nodeindex, tripleindex, triple) {
    ...
    if (tripleindex !== triple.get_representation().length) {
      ...
      if (current_fragment.contents.length > this.max_fragment_size) {
        if (root_node.node_id == current_fragment.get_root_node_id()) {
            this.balance(old_match_node)
            this.balance(new_match_node)
        } else {
  ...
  balance(node) {
    ...
    let max_node_children_node = null;
    let max_node_children = 0;
    // let half_fragment_count = this.max_fragment_size / 2
    // let closest_to_half_node = None;
    // let closest_to_half_count = this.max_fragment_size;
    let frag_nodes = fragment.request_contents()
    for (var i in frag_nodes) {
      let child_count_in_frag = frag_nodes[i].get_immediate_child_count_in_fragment();
      // let node_distance_from_half_fragment = Math.abs(half_fragment_count - frag_nodes[i].get_in_fragment_children_count() + 1);
      if (child_count_in_frag > max_node_children){
        max_node_children = child_count_in_frag;
        max_node_children_node = frag_nodes[i];
      }
      // if (node_distance_from_half_fragment < closest_to_half_count){
      //   closest_to_half_count = node_distance_from_half_fragment;
      //   closest_to_half_node = frag_nodes[i];
      // }
    }

    node = max_node_children_node;

    // node = closest_to_half_node;

    let node_children = node.get_children_objects_in_same_fragment();
    let total_changed_nodes = 0;
    for (var i = 0; i < node_children.length; i++){
      let child = node_children[i]
      let new_fragment = new Fragment(this);
      this.add_fragment(new_fragment);
      new_fragment.set_root_node(child)
      total_changed_nodes += child.change_fragment_node_and_children(fragment.fragment_id, new_fragment);
    }
    node.propagate_in_fragment_children_count(-total_changed_nodes, fragment.fragment_id)
    
    // Try to add node to parent fragment
    if (! node.has_parent_node()){ return }
    let fragment_root = node.get_parent_node().get_fragment().get_root_node();
    if (! fragment_root.has_parent_node()){ return }

    let parent_fragment_node = fragment_root.get_parent_node()
    if (fragment_root.get_in_fragment_children_count() + parent_fragment_node.get_in_fragment_children_count() > this.max_fragment_size){
      // We cannot merge with the parent node anymore
      return;
    }

    while (parent_fragment_node.has_parent_node() && parent_fragment_node.get_parent_node().get_fragment_id() == parent_fragment_node.get_fragment_id()){  
      if (fragment_root.get_in_fragment_children_count() + parent_fragment_node.get_parent_node().get_in_fragment_children_count() > this.max_fragment_size){
        // We cannot merge with parent node anymore.
        total_changed_nodes += fragment_root.change_fragment_node_and_children(fragment_root.fragment_id, parent_fragment_node.get_fragment());
        // If the parent of the upper node is still in the same fragment we need to fix his count of children in the same fragment
        if (parent_fragment_node.get_parent_node().get_fragment_id() == parent_fragment_node.fragment_id){
          parent_fragment_node.get_parent_node().propagate_in_fragment_children_count(total_changed_nodes, parent_fragment_node.fragment_id);
        }
      } else {
        parent_fragment_node = parent_fragment_node.get_parent_node()
      }
    }
  }
