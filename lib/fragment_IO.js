...
var context = {
  ...
  "children": "tree:StringCompletesRelation",
...
module.exports = class {
  ...
  encode_node(node){
    ...
    for (var childkey in node.children){
      let newchild = {}
      newchild["@id"] = "fragment" + node.children[childkey][0]+".jsonld#"+node.children[childkey][1]
      newchild["@type"] = "tree:child"
  ...
  decode_node(node){
    ...
    for (var i = 0; i < node.children.length; i++){
      ...
      let splitid = child["@id"].replace("fragment", "").replace(".jsonld", "").split("#")
