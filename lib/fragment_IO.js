...
var context = {
  ...
  "childlist": "tree:hasChildRelation",
...
module.exports = class {
  ...
  encode_node(node){
    ...
    let triple_objects = []
    let triple_representations = []
    for (var i = 0; i < node.triples.length; i++){
      let encoded_triple = this.encode_triple(node.triples[i])
      triple_objects.push(encoded_triple[0])
      triple_representations.push(encoded_triple[1])
    }
    node.triples = triple_objects;
    node.triples_metadata = triple_representations;
    ...
    for (var childkey in node.children){
      ...
      new_child_relation["tree:child"] = [{ "@id": this.getId( node.children[childkey][0], node.children[childkey][1] ) }]
  ...
  encode_suggestion(suggestion){
    ...
    for (var i = 0; i < suggestion.triples.length; i++){
      let encoded_triple = this.encode_triple(suggestion.triples[i])
      triple_list.push(encoded_triple[0])
      triple_representations.push(encoded_triple[1])
    }
    // suggestion["@graph"] = suggestion["triples"]
    // delete suggestion["triples"]
    return [triple_list, triple_representations, suggestion.score]
  }

  encode_triple(triple){
    // triple["@type"] = "Triple"
    return [triple.contents, triple.representation]
  }
  ...
  decode_fragment(fragment){
    ...
    fragment["fragment_id"] = fragment["@id"].replace(this.dataFolder + "fragment", "").replace(".jsonld", "")
  ...
  decode_node(node){
    ...
    let splitid = node["@id"].replace(this.dataFolder + "fragment", "").replace(".jsonld", "").split("#")
    ...
    for (var j = 0; j < node.suggestions_metadata.length; j++){
      ...
      for (var k = 0; k < rep_list.length; k++){
        let triple = {}
        triple["representation"] = rep_list[k]
        triple["object"] =  node.suggestions[suggestion_index]
        suggestion.triples.push(triple)
        suggestion_index += 1;
      }
    ...
    for (var j = 0; j < node.triples.length; j++){
      let triple = {}
      triple["representation"] = node.triples_metadata[j]
      triple["object"] =  node.triples[j]
      triple = this.decode_triple(triple)
      triple_list.push(triple)
    }
    node.triples = triple_list
    ...
    //       let splitid = child[j]["@id"].replace(this.dataFolder + "fragment", "").replace(".jsonld", "").split("#")
    ...
    if (node.parent_node != null){
      node.parent_node = node.parent_node["@id"].replace(this.dataFolder + "fragment", "").replace(".jsonld", "").split("#")
    }
    node["fc"] = this.fc
    // delete node["@graph"]
    delete node.triples_metadata;
  ...
  retrieveId(str){
    if (str.indexOf(".jsonld#") != -1){
      return  str.replace(this.dataFolder + "fragment", "").split(".jsonld", "")
    }
    return [str.replace(this.dataFolder + "fragment", "").replace(".jsonld", ""), null]
  }
