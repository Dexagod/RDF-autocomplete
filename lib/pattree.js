...
module.exports = class {
  ...
  constructor(max_fragment_size, max_fragment_depth) {
      ...
      if (children_tokenstring_array.length !== 0) {
        ...
        for (var i = 0; i < children_tokenstring_array.length; i++) {
          ...
          if (node_match_index === child_tokens_length) {
            ...
            found_child = true;
            node = this.total_matching_child_node(node.children[i]);
            break;
          } else if (node_match_index > 0) {
            // A partial match was found
            found_child = true;
            return this.partial_matching_child_node(node.children[i], node_match_index, index, triple);
          }
        }
        if (found_child !== true) {
