...
module.exports = class {
  ...
  constructor(token_string, fragment, parent_node, b3) {
    ...
    this.in_fragment_children_count = 0;
  }
  ...
  add_child(node) {
    ...
    if (node.get_fragment_id() == this.fragment_id){
      this.propagate_in_fragment_children_count(node.get_in_fragment_children_count() + 1, this.fragment_id)
    }
  }
  ...
  get_in_fragment_children_count(){
    return this.in_fragment_children_count;
  }
  set_in_fragment_children_count(count){
    this.in_fragment_children_count = count;
  }
  ...
  propagate_in_fragment_children_count(increment, fragment_id){
    if (this.fragment_id == fragment_id){
      this.in_fragment_children_count += increment;
      if (this.has_parent_node()){
        this.get_parent_node().propagate_in_fragment_children_count(increment, fragment_id)
      }
    } 
  }
  ...
  // Only call from replacechild at this time ,else propagation of children count is not correct. If i were to propagate on deletion and addition on change it would be double the work and useless.
  remove_child(node) {
  ...
  // The fragment value of the newchild has to be set to enable propagating the in fragment children count.
  replace_child(oldchild, newchild) {
    ...
    let difference_in_fragment = newchild.get_in_fragment_children_count() - oldchild.get_in_fragment_children_count();
    if (difference != 0){
      this.propagate_children_count(difference)
    }
    if (difference_in_fragment != 0){
      this.propagate_in_fragment_children_count(difference, newchild.get_fragment_id())
    }
  }
  ...
  get_children_objects() {
    ...
    var values = Object.keys(children).map(function(key){
      return fc.get_fragment_by_id(children[key][0]).get_node_by_id(children[key][1]);
    });
    return values;
  }

  // Returns the objects of all children in the same fragment for iteration purposes.
  get_children_objects_in_same_fragment() {
    let children = this.children;
    let frag_id = this.fragment_id;
    let fc = this.fc;
    return Object.keys(children).reduce(
      function(ids, key){ 
        if (children[key][0] === frag_id){
          ids.push(children[key][1])
        }
        return ids
      }, []
    ).map(function(node_id){
      return fc.get_fragment_by_id(frag_id).get_node_by_id(node_id);
    });
  ...
  // Returns the amount of children this node has.
  get_immediate_child_count_in_fragment(){
    let children = this.children;
    let frag_id = this.fragment_id;
    return Object.keys(children).reduce(function(prev, key){
      if (children[key][0] == frag_id){
        return prev + 1
      } else {
        return prev
  ...
  change_fragment_node_and_children(old_fragment_id, new_fragment) {
    ...
    if (current_fragment_id == old_fragment_id) {
      ...
      for (var i = 0; i < children.length; i++){
        in_frag_children_count += children[i].change_fragment_node_and_children(old_fragment_id, new_fragment)
      }
      this.in_fragment_children_count = in_frag_children_count;
      this.notify_fragment_dirty();
      return this.in_fragment_children_count + 1;
  ...
  copy_info(othernode){
    ...
    this.set_in_fragment_children_count(othernode.get_in_fragment_children_count())
    this.notify_fragment_dirty();
  }
