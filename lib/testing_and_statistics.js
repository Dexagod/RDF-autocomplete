...
var TreeWriter = require('./fragment_writer.js');
...
var calculate_average_fragments_passed = function(b3) {
  ...
  let tw = new TreeWriter();
  tw.write_fragment(b3.root_node.fragment)
...
var calculate_node_fragments_passed = function(node, distance) {
  ...
  for (var i = 0; i < node.get_children().length; i++) {
    calculate_node_fragments_passed(node.get_children()[i], newdist);
  }
}

var newB3 = new Tree(FRAGMENT_SIZE);
