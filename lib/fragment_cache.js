...
module.exports = class {
  ...
  read_fragment_from_file(fragment_id) {
    ...
    for (var i in result.contents) {
      ...
      for (var j = 0; j < result.contents[i]['triples'].length; j++) {
          Object.setPrototypeOf(result.contents[i]['triples'][j], Triple.prototype)   
      }
      let suggestion_keys =  Object.keys(result.contents[i]['suggestions']);
      for (var j = 0; j < suggestion_keys.length; j++) {
          Object.setPrototypeOf(result.contents[i]['suggestions'][suggestion_keys[j]], Suggestion.prototype)
      }
  ...
  write_fragment_batch_to_file(index_array) {
    if (this.searching == false){
      ...
      this.fragio.write_fragment_batch(mapped_array)
    } else {
