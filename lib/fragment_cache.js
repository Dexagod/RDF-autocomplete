...
module.exports = class {
  ...
  write_fragment_batch_to_file(index_array) {
    if (this.searching == false){
      ...
      const mapped_array = index_array.map( e => this.fragment_cache[e[0]])

      for (var i = 0; i < index_array.length; i++){
        delete this.fragment_cache[index_array[i][0]]
      }
  ...
  clean_cache(){
    ...
    this.write_fragment_batch_to_file(index_array)
  }

  // clear_cache(){
  //   var mappedPageFactory = new PageFactory(this.PAGE_SIZE, this.pages_directory);
  //   mappedPageFactory.deleteAllPages(function (err) {
  //     // console.log(err || 'cache cleared');
  //   });
