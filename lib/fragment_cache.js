...
module.exports = class {
  ...
  constructor(fragment_location_folder, max_cache_size = 200000){
  ...
  async add_fragment(fragment){
    ...
    if (Object.keys(this.fragment_cache) > this.max_cache_size){
      await clean_cache().catch(error => console.log(error));;
    }
  }
  ...
  async get_fragment_by_id(fragment_id){
    if (this.fragment_cache.hasOwnProperty(fragment_id)) {
      ...
      fragment.then( function(data) {
        this.add_fragment(data)
        return data;
      }).catch( (err) =>
        console.log(err)
      )
  ...
  async remove_fragment_by_id(fragment_id){
    if (this.fragment_cache.hasOwnProperty(fragment_id)){
      await write_fragment_to_file(this.fragment_cache[fragment_id]).catch(error => console.log(error));;
  ...
  async clean_cache(){
    ...
    await remove_fragment_by_id(min_key).catch(error => console.log(error));
    return ;
  }
}
